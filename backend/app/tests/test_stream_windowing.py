...
async def test_no_inference_before_complete_rawnet2_window():
    ...
    manager._queue_latest_window(session, [0.1] * (RAWNET2_WINDOW_SAMPLES - 1), chunk_seq=1)
...
async def test_windows_are_exact_stride_based_and_buffer_is_bounded():
    ...
    manager._queue_latest_window(session, [0.0] * 8, chunk_seq=1)
    assert session.pending_window == [0.0] * 8
    manager._queue_latest_window(session, [1.0] * 2, chunk_seq=2)
    assert session.window_count == 1
    manager._queue_latest_window(session, [2.0] * 1, chunk_seq=3)
    assert session.window_count == 2
    assert session.pending_window == [0.0] * 5 + [1.0] * 2 + [2.0]
    manager._queue_latest_window(session, [3.0] * 20, chunk_seq=4)
...
async def test_busy_inference_coalesces_to_newest_window_without_overlap():
    ...
    def run_inference(session_id, identity_id, window, history_synthetic_probs=None):
    ...
    manager._queue_latest_window(session, [1.0] * 2, chunk_seq=2)
    manager._queue_latest_window(session, [2.0] * 2, chunk_seq=3)
    manager._queue_latest_window(session, [3.0] * 2, chunk_seq=4)
    release.set()
    await asyncio.wait_for(session.inference_task, timeout=2)
    assert max_active == 1
    assert calls_seen[0] == [0.0] * 8
    assert calls_seen[-1] == [0.0] * 8
